...
def get_tics(pth):
    ...
    with open(pth) as i:
        for line in i:
            ...
            if line:
                line = line.split('=')
                x = line[1].strip('"').lower()
                y = line[0].strip('"').lower()
                tic_dic[x] = y
    return tic_dic
...
def read_dat(tic):
    ...
    with open(tic_path) as i:
        tic_lis = [line.strip() for line in i]
    return tic_lis
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    dic_trans = {}
    for i in tickers_lst:
        dic_trans[i] = {'exchange': tic_exchange_dic[i], 'data': [line_to_dict(line) for line in col_lst]}
    return dic_trans
...
def create_json(data_dict, pth):
    ...
    """
    with open(pth, 'w') as i:
        json.dump(data_dict, i)
    return
